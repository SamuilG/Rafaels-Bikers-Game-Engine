...
本清单是状态拆分第 1 步的产物：明确数据的读者、写者和生命周期，不在本步骤拆出新的 Player、Camera、Render、Editor 结构体。

**“当前事实”固定指实施前的 Git 提交 `b7ff397`（2026-09-18 审计）**。本轮第 2 步会收拢流程写入口，因此完成后的源码可能已不再保留表中列出的旧写法。表中的旧路径和行号用于定位迁移来源，不能作为新接口仍允许直接写字段的承诺。**“目标约定”是边界设计，不表示所有结构拆分、暂停调度和输入隔离已经实现。**

审计范围是 `Source/Runtime/UserState/GameplayState.hpp`、`UserState.hpp` 的全部数据成员，以及内嵌 `BikeTuning` 的全部 9 个成员。检索了 `Source` 内的引用，并区分活跃路径、仅定义/仅写入以及遗留未调用的 UI。表中“未发现读取”指本仓库源码消费者，不推断外部工具或未来功能。

本轮交付范围：**第 1 步是本文的完整盘点；第 2 步实施流程入口统一，并为它接入必要的模拟开关；第 3–6 步的结构拆分和完整调度重构尚未实施。** Player、Camera、Render、Editor 仍在原有结构中；不能把流程控制器接入描述为所有状态结构已经拆分，或所有输入捕获和生命周期问题已经解决。
...
另外，Renderer T 键会通过 `isAlive = true` 复活玩家；这属于相机/玩家控制权问题，应登记到后续步骤，不能因它不写流程枚举就认为所有控制权已经分离。本轮不借该发现重构玩家或相机结构。
...
这些是流程接入所需的模拟门禁，**尚未重排为“输入/UI → 命令 → 模拟 → 渲染”的完整帧调度**。当前 App 的关卡/物理在 Renderer 处理当帧 UI 之前运行，暂停动作可能在本帧已经完成一次模拟后才被处理；输入捕获也仍需按后续步骤整体整理。编辑器主动修改变换、属性或相机不等同于游戏模拟自行推进。

本接口统一流程写入和 UI 投影；玩家数据、相机结果、渲染参数、编辑器工作区仍按第 3–4 节的基线与目标清单迁移。非流程字段的写入权收窄、完整的设置保留策略以及完整输入/模拟调度重构不属于本轮已完成范围。
