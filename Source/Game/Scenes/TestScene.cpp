...
#include "../../Runtime/Event/EventSystem.hpp"
#include "../../Runtime/UserState/UserState.hpp"
#include "../../Runtime/Animation/AnimationSystem.hpp"
#include "../../Runtime/Debug/DebugRenderer.hpp"
#include "../../Runtime/AudioSystem/AudioSystem.hpp"

namespace engine {
	namespace {
		bool TryRespawnBikeInPlace(SceneManager* scene, PhysicsSystem* physics, UserState* state) {
			if (!scene || !physics || !state) {
				return false;
			}

			flecs::entity bikeEntity = scene->find_entity("Bike_0");
			...
			JPH::BodyInterface& bi = physics->GetJoltSystem()->GetBodyInterface();
			JPH::BodyID id(bikeBodyID);

			const float linVelSq = bi.GetLinearVelocity(id).LengthSq();
			const float angVelSq = bi.GetAngularVelocity(id).LengthSq();
			...
			const float currentYaw = std::atan2(-fwd.GetX(), -fwd.GetZ());
			...
			state->gameplay.isAlive = true;
			state->deathTimer = 0.0f;
			state->gameplay.isGameOver = false;
			state->bikeLeanAngle = 0.0f;
			state->bikeSteerAngle = 0.0f;
			state->thirdPersonMode = true;
			return true;
		}
	}
	...
	void TestScene::Init(RenderSystem* render, SceneManager* scene, PhysicsSystem* physics, InputSystem* input, EventSystem* eventSys, UserState* state, AnimationSystem* anima, AudioSystem* audio) {
...

		{
			...
			flecs::entity handleLEntity;  // handle_left_4  锟紺 left handlebar grip
			flecs::entity handleREntity;  // handle_right_5 锟紺 right handlebar grip
			flecs::entity pedalLEntity;   // pedal_left_8   锟紺 left pedal plate
			flecs::entity pedalREntity;   // pedal_right_9  锟紺 right pedal plate
			...
			if (charEntity.is_valid() && bikeEntity.is_valid()) {
				...
				// Left foot 锟斤拷 pedal_left_8  (pedal_left bike-local 锟斤拷 +0.30, 0.01, -0.12)
				{
					IKChainConfig c;
				...
				// Right foot 锟斤拷 pedal_right_9  (pedal_right bike-local 锟斤拷 -0.30, -0.29, +0.17)
				{
					IKChainConfig c;
				...
				// 2. 锟斤拷锟斤拷锟接筹拷锟睫改讹拷锟斤拷
				world.defer_begin();

				// 3. 执锟叫憋拷锟斤拷锟斤拷锟斤拷时锟斤拷锟叫碉拷 set 锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷效锟斤拷锟斤拷锟角憋拷压锟斤拷锟斤拷校锟?
				world.query<SkinComponent>()
					...
					if (!e.has<AnimationComponent>()) {
						...
						ac.animIndex = -1;   // no clip 锟斤拷 rest pose; IK still runs
						ac.playing = false;
						ac.looping = false;
				...
				// 4. 锟斤拷锟斤拷锟接迟ｏ拷锟斤拷锟斤拷锟较诧拷锟睫改ｏ拷锟斤拷锟斤拷时锟窖斤拷锟斤拷锟斤拷锟斤拷全锟斤拷
				world.defer_end();
		...
		//collision event test
		{
			...
			m_event->Subscribe(EventType::Collision, [this, bikeBodyIDStr](Event& e) {
				...
				if (mState->gameplay.isGameOver || !mState->gameplay.isAlive) return; // already dead, ignore further events
		...
		m_event->Subscribe(EventType::ItemCollected, [this](Event& e) {
			...
			mState->gameplay.collectedItems = col.GetCurrentTotal();
			EngineUi::LogPrint("[Collection] {}/{} collected\n",
				col.GetCurrentTotal(), mState->gameplay.totalCollectibles);

			m_audio->PlayOneShot("Collect");
		});

		m_event->Subscribe(EventType::AllItemsCollected, [this](Event& e) {
			mState->gameplay.allCollected = true;
			mState->bikeTuning.maxSpeed *= 2.0f;
			EngineUi::LogPrint("[Collection] ALL ITEMS COLLECTED 鈥?max speed unlocked!\n");
	...
	void TestScene::Update(float dt) {
		...
		if (!mState->gameplay.isAlive && mState->gameplay.remainingLives > 0) {
			if (TryRespawnBikeInPlace(m_scene, m_physics, mState)) {
				--mState->gameplay.remainingLives;
				printf("[Gameplay] Auto respawn triggered. Remaining lives: %d\n", mState->gameplay.remainingLives);
			}
		}

		if (m_input && m_input->IsActionPressed("DEPLOY") && !mState->gameplay.isAlive) {
			if (TryRespawnBikeInPlace(m_scene, m_physics, mState)) {
				printf("[Gameplay] Manual respawn triggered.\n");
			}
		}
