...
namespace engine {
	...
	public:
		...
		void Update(float dt) override
		{
			...
			const Frustum* activeFrustum = nullptr;
...

			{
				...
				if (mSceneManager && mState->frustumCullingEnabled) {
					cameraFrustum = BuildFrustum(sceneUniforms.projCam);
					activeFrustum = &cameraFrustum;
				}

				finalBatches = mSceneManager
					? mSceneManager->get_render_batches(activeFrustum, mState->frustumCullingPadding, camPosWorld)
					: std::vector<RenderBatch>{};

				if (mSceneManager) {
					mState->frustumCullingTotalCandidates = mSceneManager->get_last_frustum_culling_candidates();
					mState->frustumCullingVisibleCandidates = mSceneManager->get_last_frustum_culling_visible();
				}
			...
			// =========================================================
			// Record and    commands for this frame
			// 在 Update 函数末尾找到 record_commands 调用，修改如下：
			{
				ZoneScopedN("RecordCommands");
				record_commands(
