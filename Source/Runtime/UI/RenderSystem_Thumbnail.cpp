...
namespace engine {
	...
	// 生成模型缩略图输�?Vulkan 图像资源// Generate model thumbnail output Vulkan image resource
    void RenderSystem::GenerateModelThumbnail(const std::string& modelPath) {
        ...
        for (const auto& batch : batches) {
            ...
            vkCmdBindVertexBuffers(cmd, 3, 1, &mMeshBoneIndices[batch.meshIndex].buffer, &offset);
            vkCmdBindVertexBuffers(cmd, 4, 1, &mMeshBoneWeights[batch.meshIndex].buffer, &offset);
	...
	// 预加载模型数据以供缩略图和预览使�?/ Preload model data for thumbnail and preview usage
