...
namespace
{
	namespace local_cfg
	{
		...
		constexpr char const* ParticleTextures[] = {
			"assets/a12/models/fire.png",
			"assets/a12/models/aa.png"
		};
		//================particle system=======================
	}
...
int main() try
{
	...
	//================particle system===================================================
	// 粒子系统
	std::vector<ParticleSystem*> allParticles;
	...
	//particle textures
	for (const auto& path : local_cfg::ParticleTextures) {
		//load Image
		lut::Image img = lut::load_image_texture2d(path, window, cpool.handle, allocator, VK_FORMAT_R8G8B8A8_UNORM);

		//View
		lut::ImageView view = lut::create_image_view_texture2d(window, img.image, VK_FORMAT_R8G8B8A8_UNORM);

		//Descriptor Set
		VkDescriptorSet descSet = lut::alloc_desc_set(window, dpool.handle, objectLayout.handle);
		...
		mainImgInfo.sampler = defaultSampler.handle;

		VkDescriptorImageInfo dummyImgInfo{};
		dummyImgInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		dummyImgInfo.imageView = defaultGrayTextureView.handle;
		dummyImgInfo.sampler = defaultSampler.handle;
...
// 創建第 1 組：火焰
	...
	//貼圖設定
	fire->config.textureDescriptor = particleTextureDict[local_cfg::ParticleTextures[0]]; // 綁定貼圖
	...
	fire->init(allocator, 300, emitterPos1);
	allParticles.push_back(fire);
	}
	//創建第 2組：灰煙
	{
	...
	//貼圖設定
	smoke->config.textureDescriptor = particleTextureDict[ParticleTextures[0]]; // 綁定貼圖
	...
	smoke->init(allocator, 800, emitterPos4);
	allParticles.push_back(smoke);
	}

	// 創建第 3組：火花
	{
		ParticleSystem* magic = new ParticleSystem();
		magic->config.textureDescriptor = particleTextureDict[ParticleTextures[1]];// 綁定貼圖
		magic->config.useTexture = 1;
		magic->config.sizeMin = 500.0f;
		magic->config.sizeMax = 500.0f;
		magic->config.emitterPos = emitterPos2;
		magic->init(allocator, 1, emitterPos2);
		allParticles.push_back(magic);
	}
	
	// 創建第 4組：火焰黑
	{
		ParticleSystem* c = new ParticleSystem();
		//c->config.textureDescriptor = particleDescSet; // 綁定星星貼圖
		c->config.emitterPos = emitterPos3;
		c->init(allocator, 1000, emitterPos3);
		allParticles.push_back(c);
	}
	//================particle system===================================================
	...
	lut::Pipeline particlePipe = create_particle_pipeline( window, pipeLayout.handle, VK_FORMAT_R16G16B16A16_SFLOAT );
	...
	while( !glfwWindowShouldClose( window.window ) )
	{
		...
		{
			for (ParticleSystem* ps : allParticles)
			{
				// 直接讀取它自己身上存的位置！
				ps->update(dt, ps->config.emitterPos);
				ps->upload(allocator);
				ps->uploadDebug(allocator, ps->config.emitterPos);
			}
		}
		//================particle system===================================================
	...
	//释放粒子内存
	for (ParticleSystem* ps : allParticles) {
		ps->shutdown(allocator); // 1. 歸還 GPU 顯存
		delete ps;               // 2. 釋放 CPU 記憶體
	}
	allParticles.clear();        // 3. 清空陣列
	particleTextureDict.clear();
	particleImageViews.clear();
	particleImages.clear();
	//================particle system==============
	return 0;
}
